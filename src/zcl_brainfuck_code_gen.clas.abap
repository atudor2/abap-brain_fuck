...
CLASS zcl_brainfuck_code_gen DEFINITION
  ...
  PRIVATE SECTION.
    ...
    CLASS-METHODS make_padded_code_template_line
      IMPORTING
        i_input         TYPE string
        i_max_width     TYPE i DEFAULT 255
      RETURNING
        VALUE(r_result) TYPE zcl_brainfuck_code_gen=>t_prog_src.
ENDCLASS.
...
CLASS zcl_brainfuck_code_gen IMPLEMENTATION.
  ...
  METHOD get_generated_by.
    r_result = make_padded_code_template_line( i_max_width = i_max_width i_input = |Generated By: { cl_abap_syst=>get_user_name( ) }| ).
  ENDMETHOD.

  METHOD get_generated_at.
    r_result = make_padded_code_template_line( i_max_width = i_max_width i_input = |Generated At: { sy-datum } { sy-uzeit }| ).
  ENDMETHOD.

  METHOD make_padded_code_template_line.
    " Calculate the padding width:
    " (max line width) - input width - additional leading and trailing '*'
    DATA(width) = i_max_width - strlen( i_input ) - 4.

    r_result = |* { i_input WIDTH = width PAD = ` ` } *|.
  ENDMETHOD.
